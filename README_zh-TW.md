...
- **自動更新玩家數量頻道統計**：自動在頻道計數器中更新玩家數量或伺服器狀態。(需要在設定中填入 `STATS_CHANNEL_ID`，此功能與 `/setstatus` 不同)
...
- **多種斜線和前綴指令**：
  ...
  - `setname` - **更新嵌入訊息中使用的伺服器名稱 (僅限管理員)。**
  - `setsite` - **更新嵌入訊息中使用的伺服器網站連結 (僅限管理員)。**
  - `help` - **提供可用指令列表。**
  - `help [command]` - **發送指令的詳細資訊。**

## v1.2.0 版本新功能

此更新專注於國際化改進、開發者體驗和程式碼品質。

### v1.2.0 改進的功能
- **預設語言**：將預設語言更改為英文 (`en`) 以獲得更廣泛的適用性
- **文檔**：在整個程式碼庫中添加了全面的程式碼註釋，以便於維護
- **翻譯一致性**：驗證並修復了所有支援語言 (de, es, fr, pt, ru, uk) 中的缺失翻譯鍵值
- **錯誤修復**：解決了狀態訊息中的字符編碼問題
...
1. 複製 `docker-compose.example.yml` 到 `docker-compose.yml` (如果您還沒有這麼做)：
   ```bash
   npm run setup
   ```
2. 編輯 `docker-compose.yml` 並新增您的 Discord Token 和其他環境變數。
3. 啟動機器人：
   ```bash
   npm run docker:dev
   ```
...
| STATS_CHANNEL_ID | 玩家數量統計頻道 ID (必須在此設定，無法透過指令更改) | 無 |
...
3. **配置機器人：**
   ...
   - 編輯 `.env` 文件，填入您的設定：
     ...
     STATS_CHANNEL_ID=       # 玩家數量統計的頻道 ID (選填，若需使用則必須在此設定)
