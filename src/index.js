...
// Suppress specific DeprecationWarning from Discord.js/CommandKit
const originalEmitWarning = process.emitWarning;
process.emitWarning = (warning, ...args) => {
  // Check if it's the specific warning about ready event
  if (
    (typeof warning === 'string' && warning.includes('The ready event has been renamed to clientReady')) ||
    (warning && warning.message && warning.message.includes('The ready event has been renamed to clientReady'))
  ) {
    return;
  }
 ...
 * Supports legacy arguments and configuration overrides.
 *
 * @param {Object|boolean} input - Configuration override object OR boolean flag for dataOnly
 * @param {boolean} dataOnlyArg - If true, returns only raw data without processing player list
 */
const getServerDataAndPlayerList = async (input = null, dataOnlyArg = false) => {
  // Handle overloaded arguments to support legacy (dataOnly) call
  // Argument scenarios:
  // 1. (boolean) -> treat as dataOnly
  // 2. (object) -> treat as config override (e.g., from slash commands)
  // 3. (object, boolean) -> config override + dataOnly
  // 4. () -> default behavior (read data.json for current server)
  ...
  try {
    ...
    if (configOverride) {
      // If config override is provided (e.g. from dynamic embeds), use it directly
      // We assume configOverride is already formatted correctly with IP/Port/etc
      serverConfig = {
        ...config,
        mcserver: {
          ...config.mcserver,
...
const statusMessageEdit = async (ip, port, type, name, message, isPlayerAvatarEmoji, client) => {
  try {
    ...
    if (isOnline) {
      ...
      const { offlineStatus, errorStatus } = await import('./embeds.js');
